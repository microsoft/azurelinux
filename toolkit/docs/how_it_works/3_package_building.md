...
The tool uses the `worker_chroot` (see [Chroot Worker](1_initial_prep.md#chroot_worker)) to locate packages. The worker will search in three locations: 1) the local chroot environment, 2) already build RPMs in `./../out/RPMS/`, 3) any remote repo listed in `REPO_LIST ?= $(MANIFESTS_DIR)/package/base.repo`.
