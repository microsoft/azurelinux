...
Specifies the URL of the ISO image for the kernel to download at boot time.
...
in the ISO after downloading it.

For example:
- ```yaml
  pxe:
    isoImageUrl: http://ip/iso-publish-path/my-liveos.iso
  ```

The supported download protocols are:
- `http`

The user may omit file name from the URL and the Azure Linux Image Customizer
will append the output image name to the specified URL.

For example,
- If the user has:
  - ```yaml
    pxe:
      isoImageUrl: http://ip/iso-publish-path
    ```
- and specifies the following on the command line:
  - ```bash
    imagecustomizer ... --output-image-file xyz.iso ...
    ```
- the iso file will be downloaded from:
  - ```bash
    http://ip/iso-publish-path/xyz.iso
    ```
