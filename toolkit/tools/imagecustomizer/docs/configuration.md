...
- [config type](#config-type)
  ...
  - [pxe](#pxe-type)
    - [isoImageUrl](#isoimageurl-string)
  - [os type](#os-type)
...
- ```yaml
  pxe:
    isoImageUrl: http://hostname-or-ip/iso-publish-path/my-liveos.iso
  ```
...
will append the output image name to the specified URL. The output image name
is specified on the command-line using the `--output-image file` argument (see
the [command-line interface](./cli.md) document for more details).
...
For example,
- If the user has the following content in this configuration file:
  - ```yaml
    pxe:
      isoImageUrl: http://hostname-or-ip/iso-publish-path
    ```
