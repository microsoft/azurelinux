...
func (mp *ManualPartitionWidget) populateTable() (err error) {
	...
	bootPartitionMountPoint, _, _, err := configuration.BootPartitionConfig(mp.bootType)
	if err != nil {
		return
	}
...
func (mp *ManualPartitionWidget) unmarshalPartitionTable() (err error) {
	...
	_, bootMountOptions, bootFlags, err := configuration.BootPartitionConfig(mp.bootType)
	if err != nil {
		return
	}
