...
func TestShouldFailNonInstalledCustomKeys_PackageRepo(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Equal(t, "invalid value for package repo 'mariner-official-base' [GPGKeys] (file:///etc/pki/rpm-gpg/my-custom-key), custom GPG keys are only supported for repos that are installed", err.Error())
