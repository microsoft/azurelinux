...
func (o *OfficialScript) TransferBuiltRpms() (rpmsMoved []string, err error) {
	...
	for _, rpmName := range builtRpms {
		builtRpmsDir := filepath.Join(o.BuildDir, "toolchain", "built_rpms_all")
		archDir, rpmErr := rpm.ExtractArchFromRPMPath(rpmName)
		if rpmErr != nil {
			err = fmt.Errorf("failed to extract arch from rpm path. Error:\n%w", rpmErr)
			return
		}
		src := filepath.Join(builtRpmsDir, archDir, rpmName)
		dst := filepath.Join(o.RpmsDir, archDir, rpmName)
