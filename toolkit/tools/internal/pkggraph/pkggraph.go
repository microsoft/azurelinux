...
/*check if a runNode already exists for the same version of the remoteNode, if exists, replace remoteNode in the lookup with runNode*/
func (g *PkgGraph) AddRunToLookup(pkgNode *PkgNode) (err error) {
	var (
		requestInterval, nodeInterval pkgjson.PackageVerInterval
	)

	if pkgNode.Type != TypeLocalRun {
		logger.Log.Panicf("pkgNode %v is not a LocalRun node. This function is not supposed to be called with a non-local-run nodes. Something is seriously wrong!", pkgNode)
	}
	...
	for _, lookupNode := range samePkgNodes {
		...
		if lookupNode.RunNode.Type != TypeRemoteRun {
			continue
		}
