...
const (
	...
	kernelArgsDebugTemplateDebug = " rd.debug rd.live.debug=1 rd.retry=40 "
	kernelArgsConsoleTemplate    = " console=tty0 console=ttyS0,115200n8 "
...
func (b *LiveOSIsoBuilder) updateGrubCfg(savedConfigKernelArgsFilePath string, grubCfgFileName string, extraCommandLine string) error {
	...
	if !grubMkconfigEnabled {
		...
		logger.Log.Debugf("---- mergedExtraCommandLine=(%s)", mergedExtraCommandLine)

		liveosKernelArgs := fmt.Sprintf(kernelArgsLiveOSTemplate, liveOSDir, liveOSImage)
		additionalKernelCommandline := " " + kernelArgsConsoleTemplate + " " + liveosKernelArgs + " " + mergedExtraCommandLine
		logger.Log.Debugf("---- additionalKernelCommandline=(%s)", additionalKernelCommandline)

		inputContentString, err = appendKernelCommandLineArgs(inputContentString, additionalKernelCommandline)
		if err != nil {
	...
	logger.Log.Debugf("---- writing savedConfigKernelArgsFilePath=(%s)", savedConfigKernelArgsFilePath)
	err = os.MkdirAll(filepath.Dir(savedConfigKernelArgsFilePath), os.ModePerm)
	if err != nil {
		return fmt.Errorf("failed to create directory for (%s):\n%w", savedConfigKernelArgsFilePath, err)
	}
	err = os.WriteFile(savedConfigKernelArgsFilePath, []byte(mergedExtraCommandLine), 0o644)
	if err != nil {
		return fmt.Errorf("failed to write %s:\n%w", savedConfigKernelArgsFilePath, err)
	}

	logger.Log.Debugf("---- writing grubCfgFileName=(%s)", grubCfgFileName)
	err = os.WriteFile(grubCfgFileName, []byte(inputContentString), 0o644)
	if err != nil {
		return fmt.Errorf("failed to write %s:\n%w", grubCfgFileName, err)
	}
...
//   - extracted artifacts
func (b *LiveOSIsoBuilder) prepareLiveOSDir(inputIsoSavedKernelArgsFilePath string, writeableRootfsDir string, isoMakerArtifactsStagingDir string, extraCommandLine string) error {
	...
	if inputIsoSavedKernelArgsFilePath != "" {
		// If there is an input iso, set the kernel args file to it so that it's merged later.
		b.artifacts.savedKernelArgsFilePath = inputIsoSavedKernelArgsFilePath
		logger.Log.Debugf("---- [savedKernelArgsFilePath] using inputIsoSavedKernelArgsFilePath=(%s)", inputIsoSavedKernelArgsFilePath)
	} else {
		// Otherwise, just create a new file under this iso's artifact dir.
		b.artifacts.savedKernelArgsFilePath = filepath.Join(b.workingDirs.isoArtifactsDir, savedConfigIsoDir, savedKernelArgsFileName)
		logger.Log.Debugf("---- [savedKernelArgsFilePath] using new path =(%s)", b.artifacts.savedKernelArgsFilePath)
	}
...
func (b *LiveOSIsoBuilder) createIsoImage(additionalIsoFiles []safechroot.FileToCopy, isoOutputDir, isoOutputBaseName string) error {
	...
	logger.Log.Debugf("-- createIsoImage() - calling isomakelib")
	...
	for sourceFile, targetFile := range b.artifacts.additionalFiles {
		...
		logger.Log.Debugf("---- scheduling additional file (%s) to (%s)", sourceFile, targetFile)
	...
	logger.Log.Debugf("---- scheduling additional file savedKernelArgsFilePath (%s) to (%s)", isoKernelArgsToCopy.Src, isoKernelArgsToCopy.Dest)
...
func createLiveOSIsoImage(buildDir, baseConfigPath string, inputIsoArtifacts *LiveOSIsoBuilder, isoConfig *imagecustomizerapi.Iso, rawImageFile, outputImageDir, outputImageBase string) (err error) {
	...
	if inputIsoArtifacts != nil {
		logger.Log.Debugf("---- copying additional files from old iso")
		for inputSourceFile, inputTargetFile := range inputIsoArtifacts.artifacts.additionalFiles {

			logger.Log.Debugf("---- -- considering (%s) -> (%s)", inputSourceFile, inputTargetFile)
			...
			if !found {
				logger.Log.Debugf("---- -- not found - adding to additional files.")
				isoBuilder.artifacts.additionalFiles[inputSourceFile] = inputTargetFile
			} else {
				logger.Log.Debugf("---- -- found - not adding to additional files.")
			}
		}
	}
...
func extractIsoImageContents(buildDir string, isoImageFile string, isoExpansionFolder string) (err error) {
	...
	logger.Log.Debugf("---- extracting iso contents to: %s", isoExpansionFolder)
...
func createIsoBuilderFromIsoImage(buildDir string, buildDirAbs string, isoImageFile string) (isoBuilder *LiveOSIsoBuilder, err error) {
	...
	for _, isoFile := range isoFiles {
		...
		case savedKernelArgsFileName:
			...
			scheduleAdditionalFile = false
			logger.Log.Debugf("---- recording retained iso config: %s", isoBuilder.artifacts.savedKernelArgsFilePath)
		}
