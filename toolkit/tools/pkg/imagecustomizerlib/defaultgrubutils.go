...
const (
	...
	defaultGrubFileCmdlineLinux        = "GRUB_CMDLINE_LINUX"
	defaultGrubFileCmdlineLinuxDefault = "GRUB_CMDLINE_LINUX_DEFAULT"
	grubKernelOpts                     = "kernelopts"

	// The variable in the /etc/default/grub file that contains the SELinux args.
	defaultGrubFileVarCmdlineForSELinux = defaultGrubFileCmdlineLinux
)
...
func findGrubDefaultFileVarAssigns(grubDefaultConfig string) ([]grubDefaultVarAssign, error) {
...
func findGrubDefaultFileVarAssign(varAssigns []grubDefaultVarAssign, name string) (grubDefaultVarAssign, error) {
...
func getGrubDefaultFileLinuxArgs(defaultGrubFileContent string, varName string,
) (grubDefaultVarAssign, []grubConfigLinuxArg, int, error) {
	varAssigns, err := findGrubDefaultFileVarAssigns(defaultGrubFileContent)
	if err != nil {
	...
	cmdLineVarAssign, err := findGrubDefaultFileVarAssign(varAssigns, varName)
	if err != nil {
		return grubDefaultVarAssign{}, nil, 0, err
	}
	...
	var insertAt int
	if varName == defaultGrubFileCmdlineLinuxDefault {
...
func addExtraCommandLineToDefaultGrubFile(defaultGrubFileContent string, extraCommandLine string) (string, error) {
	cmdLineVarAssign, _, insertAt, err := getGrubDefaultFileLinuxArgs(defaultGrubFileContent,
		defaultGrubFileCmdlineLinuxDefault)
	if err != nil {
		return "", err
	}
...
func updateDefaultGrubFileKernelCommandLineArgs(defaultGrubFileContent string, varName string, argsToRemove []string,
	newArgs []string,
) (string, error) {
	cmdLineVarAssign, args, insertAt, err := getGrubDefaultFileLinuxArgs(defaultGrubFileContent, varName)
	if err != nil {
		return "", err
	}
...
func replaceDefaultGrubFileVarAssign(defaultGrubFileContent string, varAssign grubDefaultVarAssign, newValue string,
) string {
