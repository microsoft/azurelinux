...
func (cfg *Config) ScheduleBuild() error {
	if cfg.Workers <= 0 {
	...
	err = cfg.buildGraph(cfg.InputGraphFile, cfg.OutputGraphFile, agent, cfg.Workers, cfg.BuildAttempts, cfg.StopOnFailure, !cfg.NoCache, packageVersToBuild, packagesNamesToRebuild, ignoredPackages, reservedFiles, cfg.DeltaBuild)
	return err
}
...
func (cfg *Config) buildGraph(inputFile, outputFile string, agent buildagents.BuildAgent, workers, buildAttempts int, stopOnFailure, canUseCache bool, packagesToBuild []*pkgjson.PackageVer, packagesNamesToRebuild, ignoredPackages, reservedFiles []string, deltaBuild bool) (err error) {
	...
	builtGraph, err := cfg.buildAllNodes(stopOnFailure, isGraphOptimized, canUseCache, packagesNamesToRebuild, pkgGraph, &graphMutex, goalNode, channels, reservedFiles, deltaBuild)
...
// - Repeat.
func (cfg *Config) buildAllNodes(stopOnFailure, isGraphOptimized, canUseCache bool, packagesNamesToRebuild []string, pkgGraph *pkggraph.PkgGraph, graphMutex *sync.RWMutex, goalNode *pkggraph.PkgNode, channels *schedulerChannels, reservedFiles []string, deltaBuild bool) (builtGraph *pkggraph.PkgGraph, err error) {
	var (
	...
	schedulerutils.RecordBuildSummary(builtGraph, graphMutex, buildState, cfg.OutputCSVFile)
