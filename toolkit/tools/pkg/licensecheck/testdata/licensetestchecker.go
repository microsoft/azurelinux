...
func main() {
	...
	filesNotDetectedAsLicense := checkFalseNegatives(realLicenses, exceptions)
	...
	invalidDocs := checkFalsePositives(docs, exceptions)
	...
	invalidOtherFiles := checkFalsePositives(otherFiles, exceptions)
...
func checkFalseNegatives(tests testData, exceptions licensecheck.LicenseExceptions) (falseNegatives testData) {
	for _, test := range tests.TestDataEntries {
		if !licensecheck.IsALicenseFile(test.Pkg, test.Path) || exceptions.ShouldIgnoreFile(test.Pkg, test.Path) {
...
func checkFalsePositives(tests testData, exceptions licensecheck.LicenseExceptions) (falsePositives testData) {
	for _, test := range tests.TestDataEntries {
		if licensecheck.IsALicenseFile(test.Pkg, test.Path) && !exceptions.ShouldIgnoreFile(test.Pkg, test.Path) {
