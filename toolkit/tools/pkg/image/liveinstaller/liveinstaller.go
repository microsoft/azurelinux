...
func (cfg *Config) Install() error {
	...
	args := imagerArguments{
		imagerTool:  cfg.ImagerTool,
		buildDir:    cfg.BuildDir,
		baseDirPath: cfg.BaseDirPath,
		logLevel:    logger.Log.GetLevel().String(),
		logFile:     cfg.ImagerLogFile,
	}

	installFunc := installerFactory(cfg.ForceAttended, cfg.ConfigFile, cfg.TemplateConfigFile)
