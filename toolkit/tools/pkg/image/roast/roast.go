...
import (
	...
	"github.com/microsoft/CBL-Mariner/toolkit/tools/pkg/image/roast/formats"
...
func (cfg *Config) GenerateImageArtifacts() error {
	if cfg.Workers <= 0 {
		logger.Log.Panicf("Value in --workers must be greater than zero. Found %d", cfg.Workers)
	}

	inDirPath, err := filepath.Abs(cfg.InputDir)
	if err != nil {
		logger.Log.Panicf("Error when calculating input directory path: %s", err)
	}

	outDirPath, err := filepath.Abs(cfg.OutputDir)
	if err != nil {
		logger.Log.Panicf("Error when calculating absolute output path: %s", err)
	}

	tmpDirPath, err := filepath.Abs(cfg.OutputDir)
	if err != nil {
	...
	config, err := configuration.Load(cfg.ConfigFile)
	if err != nil {
		logger.Log.Panicf("Failed loading image configuration. Error: %s", err)
	}

	return generateImageArtifacts(cfg.Workers, inDirPath, outDirPath, cfg.ReleaseVersion, cfg.ImageTag, tmpDirPath, config)
}
