...
func newTemplateSrcConfig(cfg *Config) sourceRetrievalConfiguration {
	...
	return templateSrcConfig
}

func (cfg *Config) CreateAllSRPMsWrapper() error {
	templateSrcConfig := newTemplateSrcConfig(cfg)

	// A pack list may be provided, if so only pack this subset.
	// If non is provided, pack all srpms.
	packList, err := parsePackListFile(cfg.PackListFile)
	logger.PanicOnError(err)
	return cfg.createAllSRPMsWrapper(cfg.SpecsDir, cfg.DistTag, cfg.BuildDir, cfg.OutDir, cfg.WorkerTar, cfg.Workers, cfg.NestedSourcesDir, cfg.RepackAll, cfg.RunCheck, packList, templateSrcConfig)
...
func (cfg *Config) createAllSRPMsWrapper(specsDir, distTag, buildDir, outDir, workerTar string, workers int, nestedSourcesDir, repackAll, runCheck bool, packList []string, templateSrcConfig sourceRetrievalConfiguration) (err error) {
...
func createAllSRPMs(specsDir, distTag, buildDir, outDir string, workers int, nestedSourcesDir, repackAll, runCheck bool, packList []string, templateSrcConfig sourceRetrievalConfiguration) (err error) {
	...
	err = packSRPMs(specStates, distTag, buildDir, runCheck, templateSrcConfig, workers)
	return
}
...
func packSRPMs(specStates []*specState, distTag, buildDir string, runCheck bool, templateSrcConfig sourceRetrievalConfiguration, workers int) (err error) {
	var wg sync.WaitGroup
	...
	for i := 0; i < workers; i++ {
		wg.Add(1)
		go packSRPMWorker(allSpecStates, results, cancel, &wg, distTag, buildDir, runCheck, templateSrcConfig)
	}
...
func packSRPMWorker(allSpecStates <-chan *specState, results chan<- *packResult, cancel <-chan struct{}, wg *sync.WaitGroup, distTag, buildDir string, runCheck bool, templateSrcConfig sourceRetrievalConfiguration) {
	defer wg.Done()
	...
	for specState := range allSpecStates {
		...
		outputPath, err := packSingleSPEC(specState.specFile, specState.srpmFile, signaturesFilePath, buildDir, fullOutDirPath, distTag, runCheck, srcConfig)
		if err != nil {
			result.err = err
			results <- result
			continue
...
func packSingleSPEC(specFile, srpmFile, signaturesFile, buildDir, outDir, distTag string, runCheck bool, srcConfig sourceRetrievalConfiguration) (outputPath string, err error) {
