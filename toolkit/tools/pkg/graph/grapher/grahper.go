...
func (cfg *Config) GenerateDependencyGraph() (*pkggraph.PkgGraph, error) {
	const goalNodeName = "ALL"
	depGraph := pkggraph.NewPkgGraph()
	localPackages := pkgjson.PackageRepo{}

	err := localPackages.ParsePackageJSON(cfg.Input)
	if err != nil {
		return nil, err
	}

	err = populateGraph(depGraph, &localPackages, cfg.Input, cfg.StrictUnresolved)
	if err != nil {
		return nil, err
	}

	// Add a default "ALL" goal to build everything local
	_, err = depGraph.AddGoalNode(goalNodeName, nil, cfg.StrictGoals)
	if err != nil {
		return nil, err
	}
...
func addUnresolvedPackage(g *pkggraph.PkgGraph, pkgVer *pkgjson.PackageVer, strictUnresolved bool) (newRunNode *pkggraph.PkgNode, err error) {
...
// addition failed.
func addSingleDependency(g *pkggraph.PkgGraph, packageNode *pkggraph.PkgNode, dependency *pkgjson.PackageVer, strictUnresolved bool) (err error) {
	...
	if nodes == nil {
		dependentNode, err = addUnresolvedPackage(g, dependency, strictUnresolved)
		if err != nil {
...
// could not be created.
func addPkgDependencies(g *pkggraph.PkgGraph, pkg *pkgjson.Package, strictUnresolved bool) (dependenciesAdded int, err error) {
	...
	for _, dependency := range runDependencies {
		err = addSingleDependency(g, runNode, dependency, strictUnresolved)
		if err != nil {
	...
	for _, dependency := range buildDependencies {
		err = addSingleDependency(g, buildNode, dependency, strictUnresolved)
		if err != nil {
...
// the graph.
func populateGraph(graph *pkggraph.PkgGraph, repo *pkgjson.PackageRepo, input string, strictUnresolved bool) (err error) {
	packages := repo.Repo
	...
	for idx := range packages {
		pkg := packages[idx]
		num, err := addPkgDependencies(graph, pkg, strictUnresolved)
		if err != nil {
