...
func (cfg *Config) BuildSysConfig(defaultSystemConfig int) error {
	// Parse Config
	config, err := configuration.LoadWithAbsolutePaths(cfg.ConfigFile, cfg.BaseDirPath)
	logger.PanicOnError(err, "Failed to load configuration file (%s) with base directory (%s)", cfg.ConfigFile, cfg.BaseDirPath)
	...
	return cfg.buildSystemConfig(systemConfig, config.Disks, cfg.OutputDir, cfg.BuildDir)

}

func (cfg *Config) buildSystemConfig(systemConfig configuration.SystemConfig, disks []configuration.Disk, outputDir, buildDir string) (err error) {
	...
	if isRootFS {
		...
		kernelPkg, err = installutils.SelectKernelPackage(systemConfig, cfg.LiveInstallFlag)
		...
		diskDevPath, partIDToDevPathMap, partIDToFsTypeMap, isLoopDevice, encryptedRoot, readOnlyRoot, err = setupDisk(buildDir, defaultTempDiskName, cfg.LiveInstallFlag, diskConfig, systemConfig.Encryption, systemConfig.ReadOnlyVerityRoot)
		if err != nil {
			return
		}
	...
	if isOfflineInstall {
		...
		additionalExtraMountPoints := []*safechroot.MountPoint{
			safechroot.NewMountPoint(cfg.Assets, assetsMountPoint, "", safechroot.BindMountPointFlags, ""),
			safechroot.NewMountPoint(cfg.LocalRepo, localRepoMountPoint, "", safechroot.BindMountPointFlags, ""),
			safechroot.NewMountPoint(filepath.Dir(cfg.RepoFile), repoFileMountPoint, "", safechroot.BindMountPointFlags, ""),
		}
		extraMountPoints = append(extraMountPoints, additionalExtraMountPoints...)

		setupChroot := safechroot.NewChroot(setupChrootDir, existingChrootDir)
		err = setupChroot.Initialize(cfg.TdnfTar, extraDirectories, extraMountPoints)
		if err != nil {
