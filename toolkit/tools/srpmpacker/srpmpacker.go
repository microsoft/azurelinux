...
func createChroot(workerTar, buildDir, outDir, specsDir string, useAzureCliAuth bool) (chroot *safechroot.Chroot, newBuildDir, newOutDir, newSpecsDir string, err error) {
	...
	// Add .azure mount if Azure CLI auth is requested
	if useAzureCliAuth {
		...
		if homeErr == nil {
			...
			logger.Log.Warnf("Could not determine user home directory for .azure mount: %v", homeErr)
		}
	}
...
func installAzureCliPackage(chroot *safechroot.Chroot) (err error) {
	...
	logger.Log.Infof("Installing '%s' and '%s' packages for Azure CLI authenticated downloads", azureLinuxReposPackage, azureCLIPackage)
