...
func BuildNodeWorker(channels *BuildChannels, agent buildagents.BuildAgent, graphMutex *sync.RWMutex, buildAttempts int, checkAttempts int, ignoredPackages []string, tsRoot *timestamp_v2.TimeStamp) {
	...
	idleTimestamp, _ := timestamp_v2.StartMeasuringEventWithParent(tsRoot, "worker idle", 0)
	for req, cancelled := selectNextBuildRequest(channels); !cancelled && req != nil; req, cancelled = selectNextBuildRequest(channels) {
		timestamp_v2.StopMeasurementSpecific(idleTimestamp) // worker idle
		...
		case pkggraph.TypeBuild:
			ts, _ := timestamp_v2.StartMeasuringEventWithParent(tsRoot, req.Node.FriendlyName(), 0)
			...
			timestamp_v2.StopMeasurementSpecific(ts) // req.Node.FriendlyName()
