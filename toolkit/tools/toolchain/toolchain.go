...
func main() {
	...
	buildConfig, err := validateConfigOptions(*allowRebuild, *existingArchive, *useLatestAvailable, *disableCache)
	if err != nil {
	...
	if len(*summaryFile) > 0 {
		...
		logger.Log.Debugf("Changes made:\n%s", changesMade)
...
func validateConfigOptions(allowRebuild, existingArchive string, useLatestAvailable, disableCache bool) (config buildConfig, err error) {
	config = buildConfig{
		doBuild:                 false,
		doDeltaBuild:            false,
		doDownload:              false,
...
func buildBootstrapToolchainArchive(config buildConfig) (bootstrap toolchain.BootstrapScript, err error) {
	...
	if cacheOk {
		...
		logger.Log.Infof("Bootstrap cache is invalid, rebuilding bootstrap")
...
func buildOfficialToolchainArchive(config buildConfig, bootstrap toolchain.BootstrapScript, toolchainRPMs []string) (builtRpms []string, official toolchain.OfficialScript, builtArchive toolchain.Archive, err error) {
	...
	if cacheOk {
		...
		logger.Log.Infof("Official toolchain rpms cache is invalid, rebuilding toolchain")
