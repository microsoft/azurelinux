...
def check_spec_tags(base_path: str, tags: List[str], groups: List[FrozenSet], tag_expected={}) -> bool:
    ...
    for group in groups:
        ...
        for tag, specs_values in spec_tag_map.items():
            ...
            if len(set(value_list)) > 1 or (tag_expected and value_list[0] != tag_expected[tag]):
                has_error = True
                print(f'Mismatch in expected value of "{tag}":{tag_expected[tag] if tag_expected else ""}')
...
def check_matches(base_path: str):
    ...
    groups_to_check = [(['mstflintver'], mstflintver_matching_groups, None),
                       (['sdkver'], sdkver_matching_groups, None),
                       (['epoch', 'version', 'release'], version_release_matching_groups, None),
                       (['epoch', 'version'], version_matching_groups, None),
                       (['last-known-kernel'], oot_kmodule_matching_groups,
                         {'last-known-kernel' : kernel_version_release})]
