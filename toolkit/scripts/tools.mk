...
go_cmd = env GOTOOLCHAIN=auto go
...
go_current_version = $(shell cd $(TOOLS_DIR) && $(go_cmd) version | awk '{print $$3}')
...
$(TOOL_BINS_DIR)/%: $(go_common_files)
	cd $(TOOLS_DIR)/$* && \
		$(go_cmd) test -ldflags="$(go_ldflags)" -test.short -covermode=atomic -coverprofile=$(BUILD_DIR)/tools/$*.test_coverage ./... && \
		$(go_cmd) build \
			-ldflags="$(go_ldflags)" \
...
$(BUILD_DIR)/tools/internal.test_coverage: $(go_internal_files) $(go_imagegen_files) $(STATUS_FLAGS_DIR)/got_go_deps.flag
	cd $(TOOLS_DIR)/$* && \
		$(go_cmd) test -ldflags="$(go_ldflags)" -test.short -covermode=atomic -coverprofile=$@ ./...
...
$(STATUS_FLAGS_DIR)/got_go_deps.flag:
	@cd $(TOOLS_DIR)/ && \
		if [ -z "$$SUDO_USER" ]; then \
			...
			$(go_cmd) get -d ./... || echo "Failed to run 'go get', falling back to 'go build' to pull modules" ; \
		else \
			echo "SUDO_USER is set, running 'go get' as user '$$SUDO_USER'"; \
			sudo -u $$SUDO_USER $(go_cmd) get -d ./... || echo "Failed to run 'go get', falling back to 'go build' to pull modules" ; \
		fi && \
		touch $@
...
$(BUILD_DIR)/tools/all_tools.coverage: $(call shell_real_build_only, find $(TOOLS_DIR)/ -type f -name '*.go') $(STATUS_FLAGS_DIR)/got_go_deps.flag
	cd $(TOOLS_DIR) && $(go_cmd) test -ldflags="$(go_ldflags)" -coverpkg=./... -test.short -covermode=atomic -coverprofile=$@ ./...
$(test_coverage_report): $(BUILD_DIR)/tools/all_tools.coverage
	cd $(TOOLS_DIR) && $(go_cmd) tool cover -html=$(BUILD_DIR)/tools/all_tools.coverage -o $@
