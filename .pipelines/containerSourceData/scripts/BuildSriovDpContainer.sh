...
function create_sriov_dp_container_image_base {
    local componentName
...
function create_sriov_dp_container {
    mkdir -p $OUTPUT_FOLDER/CONTAINER_LISTS_FOLDER
    local dependency_component=$SRIOV_NETWORK_DEVICE_PLUGIN
    local pkgsFileName="$SRIOV_NETWORK_DEVICE_PLUGIN_NO_DASH.pkg"
    local packagesToInstall=()
    getPkgsFromFile $SRIOV_NETWORK_DEVICE_PLUGIN_NO_DASH $pkgsFileName packagesToInstall
    ...
    create_sriov_dp_container_image_base \
        "$dependency_component" \
        "$base_container_name" \
        "$base_container_tag" \
        "$packages" \
        "$CONTAINER_SRC_DIR/$SRIOV_NETWORK_DEVICE_PLUGIN_NO_DASH/Dockerfile-sriov-network-device-plugin"
