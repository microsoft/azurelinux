...
#   │   ├── distroless_debug
#   │   │   ├── distroless-debug-3.0.20240101.tar.gz
#   │   ├── distroless_minimal
#   │   │   ├── distroless-minimal-3.0.20240101.tar.gz
...
function initialization {
    ...
    cp "$CONTAINER_SRC_DIR/azurelinuxlocal.repo" "$WORK_DIR/$LOCAL_REPO_FILE"
    ...
    base_tag_tar_gz=${base_tarball_file_name##*-}           # 3.0.20240101.tar.gz
    BASE_IMAGE_TAG=${base_tag_tar_gz%.tar.gz}               # 3.0.20240101
    echo "BASE_IMAGE_TAG                -> $BASE_IMAGE_TAG"
    AZL_VERSION=${BASE_IMAGE_TAG%.*}                        # 3.0
    echo "AZL_VERSION                   -> $AZL_VERSION"
    BUILD_ID=${BASE_IMAGE_TAG##*.}                          # 20240101
