...
This module provides exhaustive testing for all anti-pattern detection 
functionality, ensuring reliable identification of spec file issues that
could affect package quality, security, or build processes.

Test Coverage:
--------------
1. **Patch File Issues** (`detect_patch_file_issues()`):
   ...
   - Proper handling of whitespace and formatting variations

2. **CVE Reference Issues** (`detect_cve_issues()`):
   - Future-dated CVEs (CVE years beyond current expected range, threshold: 2026)
   - Missing CVE documentation in changelog (CVEs referenced in spec but not in changelog)
   - Case sensitivity behavior in CVE pattern matching (regex: r'CVE-(\\d{4})-(\\d{4,})')
   - Cross-validation between description/summary and changelog sections

3. **Changelog Format Issues** (`detect_changelog_issues()`):
   ...
   - Date format validation (expects: "* Day Month DD YYYY Author <email> - Version")

4. **Integration Testing** (`detect_all()`):
   - Combined detection across all anti-pattern categories
   - End-to-end workflow validation with realistic spec file scenarios
   - Edge cases and boundary conditions
   - Performance and scalability considerations

5. **Configuration and Data Structures**:
   - Severity level mapping validation (ERROR, WARNING, INFO, CRITICAL)
   - AntiPattern dataclass functionality and field validation
   - Error handling and robustness testing
   - Configuration parameter validation

Test Structure:
---------------
Each test method follows the AAA (Arrange-Act-Assert) pattern:
1. **Arrange**: Setup test data (spec content, file lists, mock objects)
2. **Act**: Execute the detection function under test
3. **Assert**: Validate expected patterns are detected with correct properties

The test suite uses unittest framework with setUp/tearDown methods for
consistent test environment and proper resource cleanup.

Test Data Patterns:
-------------------
- Realistic spec file content matching Azure Linux conventions
- Various severity scenarios (no issues, warnings, errors, critical)
- Edge cases including malformed input, empty content, boundary conditions
...
python -m unittest test_antipattern_detector.TestAntiPatternDetector.test_missing_patch_file

# Run with verbose output
python -m unittest test_antipattern_detector.py -v

# Run integration tests only
python -m unittest test_antipattern_detector.TestAntiPatternDetector.test_detect_all_integration
```
