...
flags into any RPM package besides `azurelinux-rpm-config`, so that flag
changes are picked up automatically once `azurelinux-rpm-config` is
updated.
...
In addition, `azurelinux-rpm-config` re-selects the built-in default
...
run-time dependency on `azurelinux-rpm-config`), or use the current
