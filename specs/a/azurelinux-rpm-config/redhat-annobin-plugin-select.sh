...
then
	...
	if [ $debug -eq 1 ]
	then
	    echo "  azurelinux-rpm-config: Both plugins exist, checking version information"
	fi
	...
	then
		...
		if [ $debug -eq 1 ]
		then
		    echo "  azurelinux-rpm-config: Both plugin version files exist - comparing..."
		fi
		...
		if [ $debug -eq 1 ]
		then
		    echo "  azurelinux-rpm-config: Annobin plugin built by gcc $avers"
		    echo "  azurelinux-rpm-config: GCC     plugin built by gcc $gvers"
		fi
		...
		then
		    if [ $debug -eq 1 ]
		    then
			echo "  azurelinux-rpm-config: Both plugins built by the same compiler - using annobin-built plugin"
		    fi
		    install_annobin_version=1
		else
		    if [ $debug -eq 1 ]
		    then
			echo "  azurelinux-rpm-config: Versions differ - using gcc-built plugin"
		    fi
		...
		if [ $debug -eq 1 ]
		then
		    echo "  azurelinux-rpm-config: Annobin version file does not exist, using gcc-built plugin"
		fi
		install_gcc_version=1
	    fi
	else
		...
		if [ $debug -eq 1 ]
		then
		    echo "  azurelinux-rpm-config: GCC plugin version file does not exist, using annobin-built plugin"
		fi
		install_annobin_version=1
	    else
		if [ $debug -eq 1 ]
		then
		    echo "  azurelinux-rpm-config: Neither version file exists - playing safe and using gcc-built plugin"
		    echo "  azurelinux-rpm-config: Note: expected to find $aver and/or $gver"
		fi
		install_gcc_version=1
	    fi
	fi
	...
	if [ $debug -eq 1 ]
	then
	    echo "  azurelinux-rpm-config: Only the annobin plugin exists - using that"
	fi
	install_annobin_version=1
    fi
else
	...
	if [ $debug -eq 1 ]
	then
	    echo "  azurelinux-rpm-config: Only the gcc plugin exists - using that"
	fi
    else
	if [ $debug -eq 1 ]
	then
	    echo "  azurelinux-rpm-config: Neither plugin exists - playing safe and using gcc-built plugin"
	    echo "  azurelinux-rpm-config: Note: expected to find $aplugin and/or $gplugin"
	fi
    fi
    install_gcc_version=1
fi
...
then
    if [ $debug -eq 1 ]
    then
	echo "  azurelinux-rpm-config: Installing annobin version of $rac1"
    fi
	...
	if [ $debug -eq 1 ]
	then
	    echo "  azurelinux-rpm-config: Installing gcc version of $rac1"
	fi
	pushd $rrcdir > /dev/null
	rm -f $rac1
