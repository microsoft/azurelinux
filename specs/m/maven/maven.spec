...
echo JAVA_HOME=%{_jvmdir}/msopenjdk-25 >%{buildroot}%{_javaconfdir}/maven%{?maven_version_suffix}-openjdk25.conf

%jp_binding --verbose --variant openjdk25 --ghost maven%{?maven_version_suffix}.conf --target %{_javaconfdir}/maven%{?maven_version_suffix}-openjdk25.conf --provides %{name}-jdk-binding --requires msopenjdk-25
...
* Fri Sep 11 2026 azldev <azldev@local> - 1:3.9.11-10
