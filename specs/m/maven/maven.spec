...
%endif
BuildRequires: maven-local-msopenjdk25

Requires: %{name}-lib = %{epoch}:%{version}-%{release}
Requires: %{name}-jdk-binding
Suggests: %{name}-msopenjdk25 = %{epoch}:%{version}-%{release}
...
echo JAVA_HOME=%{_jvmdir}/msopenjdk-25 >%{buildroot}%{_javaconfdir}/maven%{?maven_version_suffix}-msopenjdk25.conf
%jp_binding --verbose --variant msopenjdk25 --ghost maven%{?maven_version_suffix}.conf --target %{_javaconfdir}/maven%{?maven_version_suffix}-msopenjdk25.conf --provides %{name}-jdk-binding --requires msopenjdk-25
%post
...
* Thu Sep 10 2026 Nan Liu <liunan@microsoft.com> - 1:3.9.11-9
- feat(java): add Microsoft OpenJDK 25 bindings

* Wed Aug 19 2026 reuben olinsky <reubeno@users.noreply.github.com> - 1:3.9.11-8
- build: mass rebuild auto-bumpable components

* Wed Aug 19 2026 reuben olinsky <reubeno@users.noreply.github.com> - 1:3.9.11-7
- build: mass rebuild auto-bumpable components

* Thu Apr 30 2026 Daniel McIlvaney <damcilva@microsoft.com> - 1:3.9.11-6
