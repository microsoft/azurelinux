## START: Set by rpmautospec
## (rpmautospec version 0.8.3)
## RPMAUTOSPEC: autorelease, autochangelog
%define autorelease(e:s:pb:n) %{?-p:0.}%{lua:
    release_number = 4;
    base_release_number = tonumber(rpm.expand("%{?-b*}%{!?-b:1}"));
    print(release_number + base_release_number - 1);
}%{?-e:.%{-e*}}%{?-s:.%{-s*}}%{!?-n:%{?dist}}
## END: Set by rpmautospec

# This spec file has been modified by azldev to include build configuration overlays.
# Do not edit manually; changes may be overwritten.

# Generated by rust2rpm 28
%bcond check 1
%global debug_package %{nil}

%global crate reqsign-azure-storage

Name:           rust-reqsign-azure-storage
Version:        3.1.0
Release:        %autorelease
Summary:        Azure Storage signing implementation for reqsign

License:        Apache-2.0
URL:            https://crates.io/crates/reqsign-azure-storage
Source:         %{crates_source}

BuildRequires:  cargo-rpm-macros >= 24

%global _description %{expand:
Azure Storage signing implementation for reqsign.}

%description %{_description}

%package        devel
Summary:        %{summary}
BuildArch:      noarch

%description    devel %{_description}

This package contains library source intended for building other packages which
use the "%{crate}" crate.

%files          devel
%license %{crate_instdir}/LICENSE
%doc %{crate_instdir}/README.md
%{crate_instdir}/

%package     -n %{name}+default-devel
Summary:        %{summary}
BuildArch:      noarch

%description -n %{name}+default-devel %{_description}

This package contains library source intended for building other packages which
use the "default" feature of the "%{crate}" crate.

%files       -n %{name}+default-devel
%ghost %{crate_instdir}/Cargo.toml

%prep
%autosetup -n %{crate}-%{version} -p1
# Remove unused mock server scripts from tests to avoid a Python dependency
rm -rv tests/mocks
%cargo_prep

%generate_buildrequires
%cargo_generate_buildrequires

%build
%cargo_build

%install
%cargo_install

%if %{with check}
%check
%cargo_test
%endif

%changelog
## START: Generated by rpmautospec
* Fri Sep 18 2026 Unknown User <please-configure-git-user@example.com> - 3.1.0-4
- Uncommitted changes

* Fri Jul 17 2026 Fedora Release Engineering <releng@fedoraproject.org> - 3.1.0-2
- Rebuilt for https://fedoraproject.org/wiki/Fedora_45_Mass_Rebuild

* Fri Jul 10 2026 Benjamin A. Beasley <code@musicinmybrain.net> - 3.1.0-1
- Update to version 3.1.0; Fixes RHBZ#2499041

* Tue Jun 09 2026 Benjamin A. Beasley <code@musicinmybrain.net> - 3.0.1-1
- Update to version 3.0.1; Fixes RHBZ#2483699

* Tue Mar 24 2026 Benjamin A. Beasley <code@musicinmybrain.net> - 3.0.0-1
- Update to version 3.0.0; Fixes RHBZ#2432771

* Sat Jan 17 2026 Fedora Release Engineering <releng@fedoraproject.org> - 2.0.1-2
- Rebuilt for https://fedoraproject.org/wiki/Fedora_44_Mass_Rebuild

* Sun Nov 09 2025 Benjamin A. Beasley <code@musicinmybrain.net> - 2.0.1-1
- Initial package (close RHBZ#2412926)
## END: Generated by rpmautospec
