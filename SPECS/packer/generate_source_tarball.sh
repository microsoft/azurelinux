...
SPEC_FOLDER="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
...
# Patches touching go.mod/go.sum must land before vendoring, otherwise the vendor
# tree would disagree with the go.mod produced at RPM build time.
patch -p1 --no-backup-if-mismatch -i "$SPEC_FOLDER/CVE-2026-45571.patch"
