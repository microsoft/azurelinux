...
# files to be removed without the main teckit-<version>/ prefix
declare -a REMOVE
REMOVE[${#REMOVE[*]}]="mac-installer/create-pkg.sh"
REMOVE[${#REMOVE[*]}]="mac-installer/Description.plist"
REMOVE[${#REMOVE[*]}]="mac-installer/.gitignore"
REMOVE[${#REMOVE[*]}]="mac-installer/Info.plist"
REMOVE[${#REMOVE[*]}]="mac-installer/Resources/License.rtf"
REMOVE[${#REMOVE[*]}]="mac-installer/Resources/ReadMe.rtf"
REMOVE[${#REMOVE[*]}]="mac-installer/Resources/Welcome.rtf"

# no changes below this line should be needed

orig="teckit-${version}"
orig_tgz="${orig}.tar.xz"
repackaged="${orig}_repackaged"
repackaged_tar="${repackaged}.tar"
repackaged_tgz="${repackaged_tar}.xz"
...
RET=0
for file in "${REMOVE[@]}"; do
    found=$(tar -Jtvf "${repackaged_tgz}" | grep "${file}")
