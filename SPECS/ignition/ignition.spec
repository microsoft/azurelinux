...
go build -ldflags "${LDFLAGS:-} -B 0x$(cat /dev/urandom | tr -d -c '0-9a-f' | head -c16)" -o ./ignition internal/main.go

%if 0%{?with_validate}
echo "Building ignition-validate..."
go build -ldflags "${LDFLAGS:-} -B 0x$(cat /dev/urandom | tr -d -c '0-9a-f' | head -c16)" -o ./ignition-validate validate/main.go
