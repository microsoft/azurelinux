...
%post
/sbin/ldconfig
# When grub2 is installed, we need to populate the defaults file, assuming that a user
# may invoke grub tools such as grub2-mkconfig for grub updates, in its current state, running
# grub2-mkconfig on Mariner will brick Mariner, as there is an assumption that the grub.cfg file
# is setup and configured only once during bootstrap of the image.

# Only update the file if the file exists and is empty, take last known good CMDLINE configuration
if [ -f /etc/default/grub ] && [ ! -s /etc/default/grub ]; then
cat << EOF > /etc/default/grub
# File generated by postscript of grub2 rpm
GRUB_TIMEOUT=$(grep -w 'set timeout' /boot/grub2/grub.cfg | awk -F'=' 'NR==1{print $NF}')
...
%verify (not md5 mtime) /etc/default/grub
%files
%defattr(-,root,root)
%license COPYING
...
%changelog
* Wed Apr 18 2023 Andy Zaugg <azaugg@linkedin.com> - 2.06-10
