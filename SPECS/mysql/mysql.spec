...
%{?with_check:BuildRequires:    shadow-utils sudo}
# Since MySQL 8.0.28
...
%if ! %{with test}
rm %{buildroot}%{_bindir}/{mysql_client_test,mysqlxtest,mysqltest_safe_process,zlib_decompress,mysql_keyring_encryption_test,mysqltest,mysqld_safe,comp_err}
rm %{buildroot}%{_mandir}/man1/zlib_decompress.1*
rm -r %{buildroot}%{_datadir}/mysql-test
rm %{buildroot}%{_libdir}/mysql/plugin/component_pfs*
rm %{buildroot}%{_libdir}/mysql/plugin/component_test*
rm %{buildroot}%{_libdir}/mysql/plugin/component_udf*
rm %{buildroot}%{_libdir}/mysql/plugin/component_example*
rm %{buildroot}%{_libdir}/mysql/plugin/libtest_*
rm %{buildroot}%{_libdir}/mysql/plugin/qa_auth_*
rm %{buildroot}%{_libdir}/mysql/plugin/test_udf_services.so
rm %{buildroot}%{_libdir}/mysql/plugin/udf_example.so
rm %{buildroot}%{_libdir}/mysql/plugin/auth.so
rm %{buildroot}%{_libdir}/mysql/plugin/auth_test_plugin.so
rm %{buildroot}%{_libdir}/mysql/plugin/component_log_sink_test.so
rm %{buildroot}%{_libdir}/mysql/plugin/component_mysqlx_global_reset.so
rm %{buildroot}%{_libdir}/mysql/plugin/libdaemon_example.so
rm %{buildroot}%{_libdir}/mysql/plugin/pfs_example_plugin_employee.so
rm %{buildroot}%{_libdir}/mysql/plugin/replication_observers_example_plugin.so
rm %{buildroot}%{_libdir}/mysql/plugin/test_security_context.so
rm %{buildroot}%{_libdir}/mysql/plugin/test_services_*
%endif
...
chown -R test:test .
%ifarch x86_64
sudo -u test ctest --force-new-ctest-process
%else
# merge_large_tests timeout in arm
sudo -u test ctest --force-new-ctest-process --exclude-regex merge_large_tests
%endif
...
(
  ...
  then
    ...
    sudo -u test perl ./mysql-test-run.pl $common_testsuite_arguments --suite=main --skip-test-list=%{skiplist} --verbose
  fi
 ...
 # If either this version wasn't marked as tested yet or I explicitly want to run the testsuite, run everything we have (~4000 test)
  ...
  then
    ...
    sudo -u test perl ./mysql-test-run.pl $common_testsuite_arguments \
...
%changelog
* Thu Apr 24 2025 Mayank Singh <mayansingh@microsoft.com> - 8.0.41-2
