...
go mod tidy
# gRPC 1.83.2 pushes the "go" directive to 1.25.0, which also moves GODEBUG defaults to the
# 1.25 baseline. That makes crypto/x509 reject the negative-serial EC-ACC root vendored by
# gosnowflake, panicking in package init and killing influxd at startup. Keep the pre-1.23
# behaviour influxdb 2.7.5 was written against.
go mod edit -godebug x509negativeserial=1
go mod vendor
