...
%{_bindir}/pathfix.py -i %{_bindir}/python3 -p -n src/spy/pcsc-spy
...
rm %{buildroot}%{_docdir}/pcsc-lite/README.DAEMON
...
%post -n pcsc-lite-libs -p /sbin/ldconfig
%postun -n pcsc-lite-libs -p /sbin/ldconfig
...
- License verified
- Removed gpg verfication
- Replace ldconfig_scriptlets with postrun commands
