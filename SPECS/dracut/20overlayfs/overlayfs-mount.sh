...
parse_kernel_cmdline_args() {
    ...
    # Retrieve the OverlayFS parameters. It is expected to be predefined by the dracut cmdline module.
    [ -n "${overlayfs}" ] || overlayfs=$(getarg rd.overlayfs=)
}
...
mount_volatile_persistent_volume() {
    ...
    if [[ "${_volume}" == "volatile" ]]; then
        ...
        if [ -f "/etc/mdadm.conf" ]; then
            # If mdadm.conf exists, assume still need to manage RAID arrays.
            if mdadm --detail "${_volume}" &> /dev/null; then
                # Assemble the RAID if it is a RAID device.
