...
%define m2_cache_tarball_name apache-%{pkg_name}-%{version}-m2.tar.gz
%define licenses_tarball_name apache-%{pkg_name}-%{version}-licenses.tar.gz
%define offline_build -o
%define _prefixmvn %{_var}/opt/apache-%{pkg_name}
...
Summary:        Apache Maven package for openjdk17
Name:           maven-jdk17
Version:        3.8.7
Release:        1%{?dist}
License:        ASL 2.0
...
Source0:        https://archive.apache.org/dist/%{pkg_name}/%{pkg_name}-3/%{version}/source/apache-%{pkg_name}-%{version}-src.tar.gz
...
Source1:        %{_mariner_sources_url}/%{pkg_name}-%{mvn_1_0_pmc_ver}.cm1.x86_64.rpm
Source2:        %{_mariner_sources_url}/%{pkg_name}-%{mvn_1_0_pmc_ver}.cm1.aarch64.rpm
...
- Initial import from maven
- Update to version 3.8.7
- Add install requirements on msopenjdk-17
