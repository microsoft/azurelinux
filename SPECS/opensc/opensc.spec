...
sed -i -e 's|%{_prefix}/local/towitoko/lib/|%{_lib}/ctapi/|' etc/opensc.conf.example.in
...
%endif
sed -i -e 's|"/lib %{_prefix}/lib\b|"/%{_lib} %{_libdir}|' configure # lib64 rpaths
...
rm -f %{buildroot}%{_sysconfdir}/opensc.conf
install -Dpm 644 etc/opensc.conf %{buildroot}%{_sysconfdir}/opensc-%{_arch}.conf
rm -f %{buildroot}%{_mandir}/man5/opensc.conf.5
install -Dpm 644 doc/files/opensc.conf.5 %{buildroot}%{_mandir}/man5/opensc-%{_arch}.conf.5
# use NEWS file timestamp as reference for configuration file
touch -r NEWS %{buildroot}%{_sysconfdir}/opensc-%{_arch}.conf
touch -r NEWS %{buildroot}%{_mandir}/man5/opensc-%{_arch}.conf.5
%else
...
rm -rf %{buildroot}%{_docdir}/opensc
...
rm -f %{buildroot}%{_mandir}/man1/opensc-notify.1*
rm -rf %{_mandir}/man1
...
%changelog
* Mon Jun 12 2023 Henry Beberman <henry.beberman@microsoft.com> - 0.22.0-2
