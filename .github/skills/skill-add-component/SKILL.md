...
- **Release calculation:** If `render` fails with "non-standard Release tag value", or if `%autorelease` is incorrectly expanded (e.g., conditional `%autorelease` specs), see [Release Configuration](../../instructions/comp-toml.instructions.md#release-configuration).
