...
- **`release.calculation = "autorelease"`?** Verify the upstream spec actually uses conditional `%autorelease` (e.g., `%if %{defined autorelease}`). If not conditional, `auto` should suffice.
