...
6. **Tenet review**: When making any non-trivial change, it is **critical** to use the tenet review skill to check for conflicts with the core tenets of the distro. Check early, and re-check often as design choices change. It is much easier to course-correct early on than to refactor later when the work is already done.
7. **Tenet review is a gate**: Treat tenet review results as a decision gate for non-trivial work. If the review returns **FAIL**, and there is no obvious resolution via design, **STOP** and escalate to the user for guidance. If the review returns **WARN**, carefully consider the warnings and consult with the user if needed before proceeding. If the review returns **PASS**, proceed but remain vigilant for any issues that may arise later on.
