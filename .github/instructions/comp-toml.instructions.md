...
- **Anchor regex overlays to the whole line, and prefer a macro toggle over a pattern match.** When `spec-search-replace` is unavoidable, anchor the pattern to the full line (e.g. `regex = '^%setup -q$'`) rather than matching a fragment, and unify several near-identical patterns into one instead of stacking multiple brittle overlays. Where the upstream spec already exposes a conditional (e.g. an `%if 0%{?rhel}` / `%if 0%{?fedora}` toggle or a definable macro), flip that macro instead of rewriting the line by regex — it survives upstream updates far better than fragment matching.
