...
- Ruff currently enforces `S101` in tests. Though that is the case, prefer `assert` over `pytest.fail(...)` and `pytest.raises(...)` for explicit value checks and exceptions.
