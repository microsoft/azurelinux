...
%global target_kernel_dist .azl4

%global target_kernel_version_full %{target_kernel_version}-%{target_kernel_release}%{target_kernel_dist}
%global kernel_source_dir /usr/src/kernels/%{target_kernel_version_full}.%{_arch}

%global target_azl_build_kernel_version %{target_kernel_version}
%global release_suffix _%{target_azl_build_kernel_version}.%{target_kernel_release}

%global KVERSION %{target_kernel_version_full}
%global K_SRC %{kernel_source_dir}
...
Source0:         MLNX_OFED_SRC-%{MLNX_OFA_DRV_SRC}.0.tgz
...
BuildRequires:  kernel-headers = %{target_kernel_version}-1%{target_kernel_dist}
...
# Set this to use SRPM as source and extract actual sources during %prep stage
%global buildsubdir %{_name}-%{_version}

%prep
# Extract the inner source tarball from the MLNX_OFED_SRC bundle:
#   outer TGZ -> SRPM -> mlnx-ofa_kernel source tgz
cd %{_builddir}
rm -rf %{_name}-%{_version}
tar xzf %{SOURCE0} --wildcards '*/SRPMS/%{_name}-%{_version}-*.src.rpm' --strip-components=2
rpm2cpio %{_name}-%{_version}-*.src.rpm | cpio -idm --quiet
rm -f %{_name}-%{_version}-*.src.rpm %{_name}.spec
tar xzf %{_name}-%{_version}.tgz
rm -f %{_name}-%{_version}.tgz
cd %{_name}-%{_version}
set -- *
mkdir source
mv "$@" source/
...
%changelog
* Wed Mar 18 2026 Zhichun Wan <zhichunwan@microsoft.com> - 26.01-1
- Upgrade version to 26.01 from NVIDIA DOCA 3.3 OFED.
- Apply Azure Linux customizations.
