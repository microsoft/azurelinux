...
# it never runs `go mod vendor`. Consequently the pinned timestamp here only affects
# reproducibility of this generation step, not the package build.
#
...
# ---------------------------------------------------------------------------
# For supply-chain reasons the vendored source tree must be reproducible (within
# toolchain tolerance). Every input that determines its contents is HARD-CODED
# below, so this script alone documents exactly how it was produced — there are
# no version/output arguments to get wrong or forget to record.
#
