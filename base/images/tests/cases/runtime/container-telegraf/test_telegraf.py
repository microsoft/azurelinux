...
def test_telegraf_version(container_exec_shell: ExecShell) -> None:
    ...
    if result.exit_code != 0:
        pytest.fail(f"telegraf --version failed: {result.output}")
    if "Telegraf" not in result.output:
        pytest.fail(f"unexpected telegraf --version output: {result.output}")
...
def test_telegraf_test_mode_emits_mem_metrics(container_exec_shell: ExecShell) -> None:
    ...
    if result.exit_code != 0:
        pytest.fail(f"telegraf --test failed: {result.output}")
    if "mem,host" not in result.output:
        pytest.fail(f"expected mem metrics in telegraf --test output: {result.output}")
...
def test_telegraf_default_command_exits_no_outputs_configured(container_exec_shell: ExecShell) -> None:
    ...
    if result.exit_code == 0:
        pytest.fail(f"expected telegraf to exit non-zero: {result.output}")
    if "no outputs found" not in result.output:
        pytest.fail(f"expected 'no outputs found' in telegraf output: {result.output}")
