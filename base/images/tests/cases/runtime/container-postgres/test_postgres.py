...
def test_postgres_default_command_reports_version(container_exec_shell: ExecShell) -> None:
    ...
    if result.exit_code != 0:
        pytest.fail(f"postgres --version failed: {result.output}")
    if "postgres (PostgreSQL)" not in result.output:
        pytest.fail(f"unexpected postgres --version output: {result.output}")
...
def test_postgres_server_starts_and_accepts_queries(container_exec_shell: ExecShell) -> None:
    ...
    if rundir.exit_code != 0:
        pytest.fail(f"failed to create {PG_RUNDIR}: {rundir.output}")
    ...
    if initdb.exit_code != 0:
        pytest.fail(f"initdb failed: {initdb.output}")

    start = container_exec_shell(f"runuser -u postgres -- pg_ctl -D {PG_DATADIR} -l /tmp/pg.log -w start")
    if start.exit_code != 0:
        pytest.fail(f"pg_ctl start failed: {start.output}")

    query = container_exec_shell("runuser -u postgres -- psql -h localhost -U postgres -c 'SELECT 1;'")
    if query.exit_code != 0:
        pytest.fail(f"psql query failed: {query.output}")
    if "1 row" not in query.output:
        pytest.fail(f"unexpected psql output: {query.output}")
