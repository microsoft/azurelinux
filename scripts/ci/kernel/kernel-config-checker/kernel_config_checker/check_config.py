...
def _get_arch_value(kernel_config: KernelConfig, architecture: str) -> str | None:
...
def check_kernel_config(
    ...
    all_configs = _collect_configs(schema.default.kernel_configs, architecture, "default")
    ...
    for config_name, config_info in all_configs.items():
        ...
        if actual_value != expected_value:
            error_msg = (
                f"  ✗ {config_name}: expected '{expected_value}', got '{actual_value}' (from {config_info['source']})"
            )
...
def check_config_across_all(schema: IntentionalKernelConfigSchema, config_name: str) -> bool:
    """Check the value of a specific config across all architectures and kernels.

    Returns True if the config is present in the schema, False otherwise.
    """
    ...
    conflicts = [arch.value for arch in all_values if len({v for _, v in all_values[arch]}) > 1]
    if conflicts:
...
def main() -> int | None:
    ...
    if args.architecture:
        ...
        if args.architecture not in valid_architectures:
            parser.error(
                f"Invalid architecture '{args.architecture}'. Expected one of: {', '.join(sorted(valid_architectures))}"
            )
    ...
    try:
        ...
        print(f"✓ Parsed kernel config: {kernel_config_path} ({len(actual_config)} settings)")
        ...
        is_valid = check_kernel_config(actual_config, schema, args.kernel_name, args.architecture)
