...
def _select_override(data: dict) -> str:
    ...
    while True:
        choice = input(f"\nSelect override (1-{len(overrides)}) or enter new name: ").strip()
        if choice.isdigit():
            idx = int(choice) - 1
...
def _add_config_to_data(data: dict, new_config: dict, override_name: str | None) -> str | None:
    ...
    if "overrides" not in data:
        data["overrides"] = []
    data["overrides"].append({"name": override_name, "kernel_configs": [new_config]})
    print(f"Created new override section: {override_name}")
    return f"'{override_name}' override section"


def add_config_interactive(schema_path: Path) -> bool:
    """Interactively add a new kernel config to the JSON file.

    Returns True if a config was added/replaced, False if the operation was
    aborted or failed validation. Callers should propagate False to the
    process exit code so scripted use can detect failure.
    """
    ...
    if config_name == "CONFIG_" or not config_name[len("CONFIG_") :].replace("_", "").isalnum():
        print("❌ Error: Invalid config name. Must be a non-empty alphanumeric symbol like CONFIG_EXAMPLE")
        return False
