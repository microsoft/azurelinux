#!/bin/bash
# Script to find missing components from a kiwi image's .packages file and add them to components.toml
#
# Usage: add-missing-components-from-image.sh <packages-file>
#
# Example:
#   ./scripts/add-missing-components-from-image.sh ./base/out/images/vm-base/azl4-vm-base.x86_64-0.1.packages
...
if [[ $# -lt 1 ]]; then
    echo "Usage: $(basename "$0") <packages-file>"
    echo ""
    echo "Arguments:"
    echo "  packages-file    Path to the .packages file generated by kiwi image build"
    echo ""
    echo "Example:"
    echo "  $(basename "$0") ./base/out/images/vm-base/azl4-vm-base.x86_64-0.1.packages"
    exit 1
fi
...
if [[ ! -f "$PACKAGES_FILE" ]]; then
    ...
    echo "Please build the image first or check the path."
    exit 1
fi
