...
# Map the test-side `kind` vocabulary used in the canonical layout
# JSON (`binary` / `debuginfo` / `srpm`) onto this script's historical
# `main` / `debuginfo` / `srpms` constants. The two vocabularies
# describe the same three things; we keep the historical constants
# below so the rest of the script (and its output relpath shape) is
# unchanged.
_LAYOUT_KIND_TO_SCRIPT_KIND = {
    "binary":    KIND_MAIN,
    "debuginfo": KIND_DEBUGINFO,
    "srpm":      KIND_SRPMS,
}
...
# layout JSON has exactly one validator implementation across the
# whole repo. Set up sys.path here (rather than using a relative
# import) because this script isn't part of any package.
...
def expand_repo_prefix(prefix: str, arches: Iterable[str]) -> list[InputRepo]:
    base = prefix.rstrip("/")
    out: list[InputRepo] = []
    for sub in load_repo_layout().subrepos:
        kind = _LAYOUT_KIND_TO_SCRIPT_KIND[sub.kind]
        if sub.per_arch:
            for arch in arches:
            ...
            out.append(InputRepo(
                kind, SRPM_ARCH, f"{base}/{sub.subpath}", "prefix",
            ))
    return out
...
def _inherit_channel(
    ...
    more channels are tied for the top spot, ``tie_break_default`` is
    preferred -- and ``tie_break_used`` is True so callers can surface
    the case explicitly rather than letting the pick masquerade as
    "the data said so". Lex sort is used as a defensive fallback if
    the configured default isn't among the tied channels (which
    shouldn't be reachable today since only ALLOWED_OUTPUT_CHANNELS
    are added to ``component_channels`` in :func:`query_azldev`, but
    the code shouldn't silently do the wrong thing if that invariant
    ever breaks).
    """
    ...
    top_count = items_sorted[0][1]
    tied = [ch for ch, n in items_sorted if n == top_count]

    if len(tied) > 1:
        if tie_break_default in tied:
            top_channel = tie_break_default
        else:
            top_channel = sorted(tied)[0]
        tie_break_used = True
    else:
        top_channel = items_sorted[0][0]
    ...
    if tie_break_used:
        reason = (
            f"inherited from sibling rpms (component={component}, "
            f"channels={{{parts}}}, tied at {top_count}, picked "
            f"{top_channel} via tie-break default)"
        )
    else:
        reason = (
            f"inherited from sibling rpms (component={component}, "
    ...
    return top_channel, reason, tie_break_used
