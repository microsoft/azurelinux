...
# AZL: upstream OpenSSH does not support GSSAPIKexAlgorithms.
# The directive is generated somewhere under python/, not necessarily openssh.py.
# Patch ALL python files: only rewrite occurrences inside string literals.
find python -type f -name "*.py" -print0 | \
  xargs -0 sed -i -E "s/([\"'])GSSAPIKexAlgorithms/\\1# GSSAPIKexAlgorithms/gI"

# Optional diagnostic (do not fail build if absent)
grep -RIn "GSSAPIKexAlgorithms" python || true
grep -RIn "# GSSAPIKexAlgorithms" python || true
...
%changelog
* Fri Dec 26 2025 Aaditya Singh <v-aditysing@microsoft.com> - 20241011-1
- Upgrade to version 20241011.
- License verified.
