...
static struct inode *tarfs_iget(struct super_block *sb, u64 ino)
{
	...
	time64_t time_sec = (((u64)disk_inode.hmtime & 0xf) << 32 ) | le32_to_cpu(disk_inode.lmtime);
	long time_ns = 0;

	inode_set_ctime(inode, time_sec, time_ns);
	inode_set_atime(inode, time_sec, time_ns);
	inode_set_mtime(inode, time_sec, time_ns);
