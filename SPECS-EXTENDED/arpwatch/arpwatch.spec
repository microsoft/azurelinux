...
if ! getent group arpwatch &> /dev/null; then
	getent group pcap 2> /dev/null | grep -q %{_vargid} &&
		/usr/sbin/groupmod -n arpwatch pcap 2> /dev/null ||
		/usr/sbin/groupadd -g %{_vargid} arpwatch 2> /dev/null
fi
if ! getent passwd arpwatch &> /dev/null; then
	getent passwd pcap 2> /dev/null | grep -q %{_varuid} &&
		/usr/sbin/usermod -l arpwatch -g %{_vargid} \
			-d %{_vararpwatch} pcap 2> /dev/null ||
		/usr/sbin/useradd -u %{_varuid} -g %{_vargid} -s /usr/sbin/nologin \
